...
{
	internal static void clt_mdct_forward(MDCTLookup l, int[] input, int input_ptr, int[] output, int output_ptr, int[] window, int overlap, int shift, int stride)
	{
	...
	internal static void clt_mdct_backward(MDCTLookup l, int[] input, int input_ptr, int[] output, int output_ptr, int[] window, int overlap, int shift, int stride)
	{
		int num = 0;
		int num2 = l.n;
		...
		int num17 = output_ptr + overlap - 1;
		num11 = output_ptr;
		int num18 = 0;
		int num19 = overlap - 1;
		for (int i = 0; i < overlap / 2; i++)
		{
			int b3 = output[num17];
			int b4 = output[num11];
			output[num11++] = Inlines.MULT16_32_Q15(window[num19], b4) - Inlines.MULT16_32_Q15(window[num18], b3);
			output[num17--] = Inlines.MULT16_32_Q15(window[num18], b4) + Inlines.MULT16_32_Q15(window[num19], b3);
			num18++;
			num19--;
		}
	}
