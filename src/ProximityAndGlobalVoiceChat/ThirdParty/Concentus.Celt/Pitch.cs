...
internal static class Pitch
{
	...
	internal static void find_best_pitch(int[] xcorr, int[] y, int len, int max_pitch, int[] best_pitch, int yshift, int maxcorr)
	{
		int num = 1;
	...
	internal static void pitch_search(int[] x_lp, int x_lp_ptr, int[] y, int len, int max_pitch, out int pitch)
	{
		int[] array = new int[2];
		int num = 0;
		...
		int a = Inlines.celt_maxabs32(array2, 0, len >> 2);
		int b = Inlines.celt_maxabs32(array3, 0, num2 >> 2);
		...
		int maxcorr = CeltPitchXCorr.pitch_xcorr(array2, array3, array4, len >> 2, max_pitch >> 2);
	...
	internal static int remove_doubling(int[] x, int maxperiod, int minperiod, int N, ref int T0_, int prev_period, int prev_gain)
	{
		...
		int num4;
		int num3 = (num4 = T0_);
		int[] array2 = new int[maxperiod + 1];
		Kernels.dual_inner_prod(x, num2, x, num2, x, num2 - num4, N, out var xy, out var xy2);
		array2[0] = xy;
		int num5 = xy;
		...
		for (int j = 2; j <= 15; j++)
		{
			...
			Kernels.dual_inner_prod(x, num2, x, num2 - num12, x, num2 - num13, N, out xy2, out var xy3);
			xy2 += xy3;
			num5 = array2[num12] + array2[num13];
		...
		for (int j = 0; j < 3; j++)
		{
			array[j] = Kernels.celt_inner_prod(x, num2, x, num2 - (num3 + j - 1), N);
		}
		int num19 = ((array[2] - array[0] > Inlines.MULT16_32_Q15((short)22938, array[1] - array[0])) ? 1 : ((array[0] - array[2] > Inlines.MULT16_32_Q15((short)22938, array[1] - array[2])) ? (-1) : 0));
		if (num18 > num10)
		{
			num18 = num10;
		}
