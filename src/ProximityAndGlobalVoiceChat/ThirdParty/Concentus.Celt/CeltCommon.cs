...
internal class CeltCommon
{
	...
	internal static void celt_preemphasis(short[] pcmp, int[] inp, int inp_ptr, int N, int CC, int upsample, int[] coef, BoxedValueInt mem, int clip)
	{
		int a = coef[0];
		int num = mem.Val;
		if (coef[1] == 0 && upsample == 1 && clip == 0)
	...
	internal static void tf_encode(int start, int end, int isTransient, int[] tf_res, int LM, int tf_select, EntropyCoder enc)
	{
		...
		for (int i = start; i < end; i++)
		{
			if (num2 + num3 <= num)
			{
				enc.enc_bit_logp(tf_res[i] ^ num5, (uint)num3);
				num2 = (uint)enc.tell();
				num5 = tf_res[i];
				num6 |= num5;
			}
	...
	internal static int alloc_trim_analysis(CeltMode m, int[][] X, int[][] bandLogE, int end, int LM, int C, AnalysisInfo analysis, ref int stereo_saving, int tf_estimate, int intensity, int surround_trim)
	{
		...
		if (C == 2)
		{
			...
			{
				int a2 = Kernels.celt_inner_prod(X[0], m.eBands[i] << LM, X[1], m.eBands[i] << LM, m.eBands[i + 1] - m.eBands[i] << LM);
	...
	internal static int stereo_analysis(CeltMode m, int[][] X, int LM)
	{
		...
		if (Inlines.MULT16_32_Q15((m.eBands[13] << LM + 1) + num5, num2) <= Inlines.MULT16_32_Q15(m.eBands[13] << LM + 1, num))
		{
			return 0;
		}
		return 1;
	...
	internal static void tf_decode(int start, int end, int isTransient, int[] tf_res, int LM, EntropyCoder dec)
	{
		...
		for (int i = start; i < end; i++)
		{
			if (num2 + num3 <= num)
			{
				num6 ^= dec.dec_bit_logp((uint)num3);
				num2 = (uint)dec.tell();
				num5 |= num6;
			}
			tf_res[i] = num6;
