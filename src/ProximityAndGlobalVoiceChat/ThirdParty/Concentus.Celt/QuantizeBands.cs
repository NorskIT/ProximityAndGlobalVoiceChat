...
internal static class QuantizeBands
{
	...
	internal static int quant_coarse_energy_impl(CeltMode m, int start, int end, int[][] eBands, int[][] oldEBands, int budget, int tell, byte[] prob_model, int[][] error, EntropyCoder enc, int C, int LM, int intra, int max_decay, int lfe)
	{
		int num = 0;
		int[] array = new int[2];
		if (tell + 3 <= budget)
		{
			enc.enc_bit_logp(intra, 3u);
		}
		int a;
		int a2;
		if (intra != 0)
		...
		for (int i = start; i < end; i++)
		{
			...
			do
			{
				...
				if (budget - tell >= 15)
				{
					...
					Laplace.ec_laplace_encode(enc, ref value, (uint)(prob_model[num8] << 7), prob_model[num8 + 1] << 6);
				}
				else if (budget - tell >= 2)
				{
					value = Inlines.IMAX(-1, Inlines.IMIN(value, 1));
					enc.enc_icdf((2 * value) ^ (0 - ((value < 0) ? 1 : 0)), small_energy_icdf, 2u);
				}
				else if (budget - tell >= 1)
				{
					value = Inlines.IMIN(0, value);
					enc.enc_bit_logp(-value, 1u);
				}
				else
				{
					value = -1;
	...
	internal static void quant_coarse_energy(CeltMode m, int start, int end, int effEnd, int[][] eBands, int[][] oldEBands, uint budget, int[][] error, EntropyCoder enc, int C, int LM, int nbAvailableBytes, int force_intra, ref int delayedIntra, int two_pass, int loss_rate, int lfe)
	{
		...
		Array.Copy(oldEBands[0], 0, array[0], 0, m.nbEBands);
		if (C == 2)
		{
			Array.Copy(oldEBands[1], 0, array[1], 0, m.nbEBands);
		}
		if (two_pass != 0 || num2 != 0)
		{
			num = quant_coarse_energy_impl(m, start, end, eBands, array, (int)budget, (int)num5, Tables.e_prob_model[LM][1], array2, enc, C, LM, 1, num6, lfe);
		}
		if (num2 == 0)
		{
			...
			int num10 = entropyCoder2.buf_ptr + (int)num8;
			uint num11 = num9 - num8;
			if (num11 != 0)
			{
				array3 = new byte[num11];
				Array.Copy(entropyCoder2.buf, num10, array3, 0, (int)num11);
			}
			enc.Assign(entropyCoder);
			int num12 = quant_coarse_energy_impl(m, start, end, eBands, oldEBands, (int)budget, (int)num5, Tables.e_prob_model[LM][num2], error, enc, C, LM, 0, num6, lfe);
			if (two_pass != 0 && (num < num12 || (num == num12 && (int)enc.tell_frac() + num3 > num7)))
			{
				enc.Assign(entropyCoder2);
				if (array3 != null)
				{
					Array.Copy(array3, 0, entropyCoder2.buf, num10, (int)(num9 - num8));
				}
				Array.Copy(array[0], 0, oldEBands[0], 0, m.nbEBands);
				Array.Copy(array2[0], 0, error[0], 0, m.nbEBands);
				if (C == 2)
				{
					Array.Copy(array[1], 0, oldEBands[1], 0, m.nbEBands);
					Array.Copy(array2[1], 0, error[1], 0, m.nbEBands);
				}
				num2 = 1;
			}
		}
	...
	internal static void quant_fine_energy(CeltMode m, int start, int end, int[][] oldEBands, int[][] error, int[] fine_quant, EntropyCoder enc, int C)
	{
		for (int i = start; i < end; i++)
		{
			...
			do
			{
				...
				enc.enc_bits((uint)num3, (uint)fine_quant[i]);
		...
		for (int i = start; i < end; i++)
		{
			...
			do
			{
				...
				if (num - num3 >= 15)
				{
					...
					a3 = Laplace.ec_laplace_decode(dec, (uint)(array[num4] << 7), array[num4 + 1] << 6);
				}
				else if (num - num3 < 2)
				{
					a3 = ((num - num3 < 1) ? (-1) : (-dec.dec_bit_logp(1u)));
				}
				else
				{
					a3 = dec.dec_icdf(small_energy_icdf, 2u);
					a3 = (a3 >> 1) ^ -(a3 & 1);
				}
	...
	internal static void unquant_fine_energy(CeltMode m, int start, int end, int[] oldEBands, int[] fine_quant, EntropyCoder dec, int C)
	{
		for (int i = start; i < end; i++)
		{
			if (fine_quant[i] > 0)
			{
				...
				do
				{
					int num2 = Inlines.SUB16(Inlines.SHR32(Inlines.SHL32((int)dec.dec_bits((uint)fine_quant[i]), 10) + 512, fine_quant[i]), 512);
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = start; j < end; j++)
			{
				...
				if (fine_quant[j] < 8 && fine_priority[j] == i)
				{
					...
					do
					{
						int num2 = Inlines.SHR16(Inlines.SHL16((int)dec.dec_bits(1u), 10) - 512, fine_quant[j] + 1);
	...
	internal static void amp2Log2(CeltMode m, int effEnd, int end, int[] bandE, int[] bandLogE, int bandLogE_ptr, int C)
	{
		int num = 0;
		do
		{
