...
internal static class Rate
{
	...
	internal static int interp_bits2pulses(CeltMode m, int start, int end, int skip_start, int[] bits1, int[] bits2, int[] thresh, int[] cap, int total, out int _balance, int skip_rsv, ref int intensity, int intensity_rsv, ref int dual_stereo, int dual_stereo_rsv, int[] bits, int[] ebits, int[] fine_priority, int C, int LM, EntropyCoder ec, int encode, int prev, int signalBandwidth)
	{
		...
		while (true)
		{
			...
			if (num17 >= Inlines.IMAX(thresh[num10], num2 + 8))
			{
				if (encode != 0)
				{
					...
					{
						ec.enc_bit_logp(1, 1u);
						break;
					}
					ec.enc_bit_logp(0, 1u);
				}
				else if (ec.dec_bit_logp(1u) != 0)
				{
					break;
				}
				num8 += 8;
		...
		if (intensity_rsv > 0)
		{
			if (encode != 0)
			{
				...
				ec.enc_uint((uint)(intensity - start), (uint)(num + 1 - start));
			}
			else
			{
				intensity = start + (int)ec.dec_uint((uint)(num + 1 - start));
			}
		}
		else
		{
		...
		if (dual_stereo_rsv > 0)
		{
			if (encode != 0)
			{
				ec.enc_bit_logp(dual_stereo, 1u);
			}
			else
			{
				dual_stereo = ec.dec_bit_logp(1u);
			}
		}
		else
		{
	...
	internal static int compute_allocation(CeltMode m, int start, int end, int[] offsets, int[] cap, int alloc_trim, ref int intensity, ref int dual_stereo, int total, out int balance, int[] pulses, int[] ebits, int[] fine_priority, int C, int LM, EntropyCoder ec, int encode, int prev, int signalBandwidth)
	{
		...
		return interp_bits2pulses(m, start, end, skip_start, array, array2, array3, cap, total, out balance, num, ref intensity, num2, ref dual_stereo, num3, pulses, ebits, fine_priority, C, LM, ec, encode, prev, signalBandwidth);
	}
}
