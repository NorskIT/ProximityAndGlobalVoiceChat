...
internal class CeltEncoder
{
	...
	internal int run_prefilter(int[][] input, int[][] prefilter_mem, int CC, int N, int prefilter_tapset, out int pitch, out int gain, out int qgain, int enabled, int nbAvailableBytes)
	{
		...
		do
		{
			Array.Copy(prefilter_mem[num], 0, array[num], 0, 1024);
			Array.Copy(input[num], overlap, array[num], 1024, N);
		}
		while (++num < CC);
		int pitch2;
		int b;
		...
		do
		{
			...
			Array.Copy(in_mem[num], 0, input[num], 0, overlap);
			if (num4 != 0)
			{
			...
			Array.Copy(input[num], N, in_mem[num], 0, overlap);
			if (N > 1024)
			{
				Array.Copy(array[num], N, prefilter_mem[num], 0, 1024);
				continue;
			}
			Arrays.MemMove(prefilter_mem[num], N, 0, 1024 - N);
			Array.Copy(array[num], 1024, prefilter_mem[num], 1024 - N, N);
		}
		while (++num < CC);
		gain = b;
		pitch = pitch2;
	...
	internal int celt_encode_with_ec(short[] pcm, int pcm_ptr, int frame_size, byte[] compressed, int compressed_ptr, int nbCompressedBytes, EntropyCoder enc)
	{
		...
		int num28 = run_prefilter(array, prefilter_mem, num3, num14, num5, out pitch, out gain, out var qgain, enabled, num17);
		...
		if (num28 == 0)
		{
			...
			{
				enc.enc_bit_logp(0, 1u);
			}
		}
		else
		{
			enc.enc_bit_logp(1, 1u);
			pitch++;
			int num29 = Inlines.EC_ILOG((uint)pitch) - 5;
			enc.enc_uint((uint)num29, 6u);
			enc.enc_bits((uint)(pitch - (16 << num29)), (uint)(4 + num29));
			pitch--;
			enc.enc_bits((uint)qgain, 3u);
			enc.enc_icdf(num5, Tables.tapset_icdf, 2u);
		}
		num2 = 0;
		num = 0;
		...
		if (num30 == 0)
		{
			Array.Copy(array4[0], array5[0], nbEBands);
			if (num4 == 2)
			{
				Array.Copy(array4[1], array5[1], nbEBands);
			}
		}
		...
		QuantizeBands.quant_coarse_energy(celtMode, num11, num12, num25, array4, oldBandE, (uint)num24, error, enc, num4, i, num17, force_intra, ref delayedIntra, (complexity >= 4) ? 1 : 0, loss_rate, lfe);
		CeltCommon.tf_encode(num11, num12, num2, array8, i, tf_select, enc);
		if (enc.tell() + 4 <= num24)
		{
			if (lfe != 0)
			{
			...
			enc.enc_icdf(spread_decision, Tables.spread_icdf, 5u);
		}
		int[] array9 = new int[nbEBands];
		int maxDepth = CeltCommon.dynalloc_analysis(array4, array5, nbEBands, num11, num12, num4, array9, lsb_depth, celtMode.logN, num2, vbr, constrained_vbr, eBands, i, num20, out var tot_boost_, lfe, array6);
		if (lfe != 0)
		{
		...
		for (int j = num11; j < num12; j++)
		{
			...
			while (num15 + (num46 << 3) < num24 - num43 && num47 < array10[j])
			{
				...
				enc.enc_bit_logp(num49, (uint)num46);
				num15 = (int)enc.tell_frac();
				if (num49 == 0)
				{
					break;
		...
		if (num15 + 48 <= num24 - num43)
		{
			...
			enc.enc_icdf(num50, Tables.trim_icdf, 7u);
			num15 = (int)enc.tell_frac();
		}
		if (num19 > 0)
		{
		...
		int num58 = Rate.compute_allocation(celtMode, num11, num12, array9, array10, num50, ref intensity, ref dual_stereo, num56, out var balance, pulses, array11, fine_priority, num4, i, enc, 1, lastCodedBands, signalBandwidth);
		if (lastCodedBands != 0)
		{
		...
		QuantizeBands.quant_fine_energy(celtMode, num11, num12, oldBandE, error, array11, enc, num4);
		byte[] collapse_masks = new byte[num4 * nbEBands];
		Bands.quant_all_bands(1, celtMode, num11, num12, array7[0], (num4 == 2) ? array7[1] : null, collapse_masks, array3, pulses, num, spread_decision, dual_stereo, intensity, array8, nbCompressedBytes * 64 - num57, balance, enc, i, num58, ref rng);
		if (num57 > 0)
		{
			num6 = ((consec_transient < 2) ? 1 : 0);
			enc.enc_bits((uint)num6, 1u);
		}
		QuantizeBands.quant_energy_finalise(celtMode, num11, num12, oldBandE, error, array11, fine_priority, nbCompressedBytes * 8 - enc.tell(), enc, num4);
		if (num7 != 0)
		{
			for (int j = 0; j < nbEBands; j++)
			{
