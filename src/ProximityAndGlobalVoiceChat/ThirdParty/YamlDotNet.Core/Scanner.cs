...
internal class Scanner : IScanner
{
	...
	private void FetchNextToken()
	{
		...
		else
		{
			...
			if (!analyzer.IsWhiteBreakOrZero())
			{
				...
				Skip();
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning for the next token, found character that cannot start any token.");
			}
			Skip();
		}
	}
	...
	private Token? ScanDirective()
	{
		...
		else
		{
			...
			{
				throw new SemanticErrorException(in start, cursor.Mark(), "While scanning a version directive, did not find preceding <document end>.");
			}
		...
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a directive, did not find expected comment or line break.");
		}
		if (analyzer.IsBreak())
		{
			SkipLine();
	...
	private void FetchDocumentIndicator(bool isStartToken)
	{
		...
		if (isStartToken)
		{
			tokens.Enqueue(new DocumentStart(in start, cursor.Mark()));
			return;
		}
		Token token = null;
	...
	private void FetchBlockEntry()
	{
		...
		Mark start = cursor.Mark();
		Skip();
		tokens.Enqueue(new BlockEntry(in start, cursor.Mark()));
	}
	...
	private void FetchKey()
	{
		...
		Skip();
		tokens.Enqueue(new Key(in start2, cursor.Mark()));
	}
	...
	private void FetchValue()
	{
		...
		Mark start = cursor.Mark();
		Skip();
		tokens.Enqueue(new Value(in start, cursor.Mark()));
	}
	...
	private Token ScanAnchor(bool isAlias)
	{
		...
		try
		{
			...
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning an anchor or alias, found value containing disallowed: []{},");
			}
	...
	private Tag ScanTag()
	{
		...
		if (analyzer.Check('<', 1))
		{
			...
			if (!analyzer.Check('>'))
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, did not find the expected '>'.");
			}
			Skip();
		}
		else
		...
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, did not find expected whitespace, comma or line break.");
		}
				...
				try
				{
					...
					if (analyzer.Check("+-"))
					{
						...
						if (analyzer.IsDigit())
						{
							if (analyzer.Check('0'))
							{
								throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a block scalar, found an indentation indicator equal to 0.");
							}
					...
					if (analyzer.Check('#'))
					{
						throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a block scalar, found a comment without whtespace after '>' indicator.");
					}
					while (analyzer.IsWhite())
					{
						Skip();
					...
					if (!analyzer.IsBreakOrZero())
					{
						throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a block scalar, did not find expected comment or line break.");
					}
					if (analyzer.IsBreak())
					{
						SkipLine();
					...
					currentIndent = ScanBlockScalarBreaks(currentIndent, builder3, isLiteral, ref end, ref isFirstLine);
					isFirstLine = false;
					...
					return new Scalar(builder.ToString(), style, start, end);
				}
				finally
				{
					((IDisposable)builderWrapper3/*cast due to constrained. prefix*/).Dispose();
				}
			}
			finally
			{
				((IDisposable)builderWrapper2/*cast due to constrained. prefix*/).Dispose();
			}
		}
		finally
		{
	...
	private long ScanBlockScalarBreaks(long currentIndent, StringBuilder breaks, bool isLiteral, ref Mark end, ref bool? isFirstLine)
	{
		...
		{
			throw new SemanticErrorException(in end, cursor.Mark(), "While scanning a literal block scalar, found extra spaces in first line.");
		}
		if (!isLiteral && num > cursor.LineOffset && num2 > -1)
		{
			throw new SemanticErrorException(in end, cursor.Mark(), "While scanning a literal block scalar, found more spaces in lines above first content line.");
		}
				...
				try
				{
					...
					try
					{
						...
						while (true)
						{
							...
							{
								throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a quoted scalar, found unexpected document indicator.");
							}
							if (analyzer.IsZero())
							{
								throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a quoted scalar, found unexpected end of stream.");
							}
							if (flag && !isSingleQuoted && indent >= cursor.LineOffset)
							{
								throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a multi-line double-quoted scalar, found wrong indentation.");
							}
							flag = false;
							while (!analyzer.IsWhiteBreakOrZero())
							{
								...
								if (!isSingleQuoted && analyzer.Check('\\'))
								{
									...
									default:
									{
										...
										throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a quoted scalar, found unknown escape character.");
									}
									}
									Skip();
									Skip();
									...
									for (int i = 0; i < num; i++)
									{
										if (!analyzer.IsHex(i))
										{
											throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a quoted scalar, did not find expected hexadecimal number.");
										}
									...
									if (num2 >= 55296 && num2 <= 57343)
									{
										...
										{
											throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a quoted scalar, found invalid Unicode surrogates.");
										}
										Skip();
									...
									else
									{
										if (num2 > 1114111)
										{
											throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a quoted scalar, found invalid Unicode character escape code.");
										}
										for (int m = 0; m < num; m++)
										{
											Skip();
					...
					{
						((IDisposable)builderWrapper4/*cast due to constrained. prefix*/).Dispose();
					}
				}
				finally
				{
						...
						while (!IsDocumentIndicator())
						{
							...
							while (analyzer.IsWhite() || analyzer.IsBreak())
							{
								if (analyzer.IsWhite())
								{
									...
									{
										throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a plain scalar, found a tab character that violate indentation.");
									}
									if (!flag)
									{
										builder2.Append(ReadCurrentCharacter());
	...
	private string ScanDirectiveName(in Mark start)
	{
		...
		try
		{
			...
			if (builder.Length == 0)
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a directive, could not find expected directive name.");
			}
			if (analyzer.EndOfInput)
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a directive, found unexpected end of stream.");
			}
			if (!analyzer.IsWhiteBreakOrZero())
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a directive, found unexpected non-alphabetical character.");
			}
	...
	private VersionDirective ScanVersionDirectiveValue(in Mark start)
	{
		...
		if (!analyzer.Check('.'))
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a %YAML directive, did not find expected digit or '.' character.");
		}
		Skip();
	...
	private TagDirective ScanTagDirectiveValue(in Mark start)
	{
		...
		if (!analyzer.IsWhite())
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a %TAG directive, did not find expected whitespace.");
		}
		SkipWhitespaces();
		string prefix = ScanTagUri(null, start);
		if (!analyzer.IsWhiteBreakOrZero())
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a %TAG directive, did not find expected whitespace or line break.");
		}
	...
	private string ScanUriEscapes(in Mark start)
	{
		...
		do
		{
			...
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, did not find URI escaped octet.");
			}
			...
			if (num == 0)
			{
				...
				if (num == 0)
				{
					throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, found an incorrect leading UTF-8 octet.");
				}
				array = new byte[num];
			}
			else if ((num2 & 0xC0) != 128)
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, found an incorrect trailing UTF-8 octet.");
			}
		...
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, found an incorrect UTF-8 sequence.");
		}
		return text;
	}
	...
	private string ScanTagHandle(bool isDirective, Mark start)
	{
		if (!analyzer.Check('!'))
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag, did not find expected '!'.");
		}
		StringBuilderPool.BuilderWrapper builderWrapper = StringBuilderPool.Rent();
		try
		{
			...
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a tag directive, did not find expected '!'.");
			}
			return builder.ToString();
		}
		finally
	...
	private int ScanVersionDirectiveNumber(in Mark start)
	{
		...
		while (analyzer.IsDigit())
		{
			if (++num2 > 9)
			{
				throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a %YAML directive, found extremely long version number.");
			}
		...
		if (num2 == 0)
		{
			throw new SyntaxErrorException(in start, cursor.Mark(), "While scanning a %YAML directive, did not find expected version number.");
		}
		return num;
	}
