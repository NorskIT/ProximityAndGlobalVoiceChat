...
internal class SilkChannelDecoder
{
	...
	internal int silk_decode_frame(EntropyCoder psRangeDec, short[] pOut, int pOut_ptr, BoxedValueInt pN, int lostFlag, int condCoding)
	{
		...
		if (lostFlag == 0 || (lostFlag == 2 && LBRR_flags[nFramesDecoded] == 1))
		{
			...
			DecodeIndices.silk_decode_indices(this, psRangeDec, nFramesDecoded, lostFlag, condCoding);
			DecodePulses.silk_decode_pulses(psRangeDec, pulses, indices.signalType, indices.quantOffsetType, frame_length);
		...
		Array.Copy(pOut, pOut_ptr, outBuf, num2, frame_length);
