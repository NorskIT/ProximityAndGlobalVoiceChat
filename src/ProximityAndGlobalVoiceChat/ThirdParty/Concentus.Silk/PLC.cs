...
internal static class PLC
{
	...
	internal static void silk_PLC(SilkChannelDecoder psDec, SilkDecoderControl psDecCtrl, short[] frame, int frame_ptr, int lost)
	{
	...
	internal static void silk_PLC_update(SilkChannelDecoder psDec, SilkDecoderControl psDecCtrl)
	{
		...
		if (psDec.indices.signalType == 2)
		{
			for (int i = 0; i * psDec.subfr_length < psDecCtrl.pitchL[psDec.nb_subfr - 1] && i != psDec.nb_subfr; i++)
			{
				...
				{
					num = num2;
					Array.Copy(psDecCtrl.LTPCoef_Q14, Inlines.silk_SMULBB(psDec.nb_subfr - 1 - i, 5), sPLC.LTPCoef_Q14, 0, 5);
		...
		Array.Copy(psDecCtrl.PredCoef_Q12[1], sPLC.prevLPC_Q12, psDec.LPC_order);
		sPLC.prevLTP_scale_Q14 = (short)psDecCtrl.LTP_scale_Q14;
		Array.Copy(psDecCtrl.Gains_Q16, psDec.nb_subfr - 2, sPLC.prevGain_Q16, 0, 2);
	...
	internal static void silk_PLC_conceal(SilkChannelDecoder psDec, SilkDecoderControl psDecCtrl, short[] frame, int frame_ptr)
	{
		...
		Array.Copy(psDec.sLPC_Q14_buf, 0, array2, num8, 16);
		...
		Array.Copy(array2, num8 + psDec.frame_length, psDec.sLPC_Q14_buf, 0, 16);
		sPLC.rand_seed = num3;
	...
	internal static void silk_PLC_glue_frames(SilkChannelDecoder psDec, short[] frame, int frame_ptr, int length)
	{
		PLCStruct sPLC = psDec.sPLC;
		if (psDec.lossCnt != 0)
		{
